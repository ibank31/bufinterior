...
export function normalizeSitePath(path = "/") {
  ...
  // Situs di-deploy dengan trailingSlash: true, jadi URL kanonik halaman selalu
  // diakhiri "/". Tambahkan trailing slash untuk path halaman agar sitemap &
  // canonical cocok dengan URL yang benar-benar di-serve (menghindari 308 redirect).
  // Kecualikan path file (mis. /sitemap.xml, /robots.txt) yang tidak boleh berakhiran slash.
  const lastSegment = cleanPathname.split("/").pop() || "";
  const isFilePath = lastSegment.includes(".");
  const withTrailingSlash =
    cleanPathname === "/" || isFilePath ? cleanPathname : `${cleanPathname}/`;

  return `${withTrailingSlash}${query}${hash}`;
}
